...
__ = {
  ...
  extractFromNode(commandLine, node) {
    ...
    // time echo hello
    if (commandName === 'time') {
      const commandAfterTime = __.extractTextRange(
        commandLine,
        node.suffix[0].pos,
        commandLine.length,
      );
      return extractSimpleCommands(commandAfterTime);
    }
    ...
    // sh -c 'echo hello' / zsh -c 'echo hello'
    if (commandName === 'sh' || commandName === 'zsh') {
