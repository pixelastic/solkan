...
__ = {
  ...
  extractFromNode(commandLine, node) {
    ...
    if (node.type === 'For' || node.type === 'ArithmeticFor') {
      return __.extractFromFor(commandLine, node);
    }
  ...
  /**
   * Extract commands from a For loop
   * Example: for i in a b c; do echo $i; done
   * @param {string} commandLine - The full command line
   * @param {object} forNode - The For node from AST
   * @returns {Array<string>} Array of commands from body
   */
  extractFromFor(commandLine, forNode) {
    // Body: echo $i
    const bodyCommands = _.chain(forNode.body.commands)
      .map((statement) => __.extractFromNode(commandLine, statement.command))
      .flatten()
      .value();
