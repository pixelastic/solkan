...
__ = {
  ...
  extractFromNode(commandLine, node) {
    ...
    // rtk --version / rtk rewrite / rtk echo hello
    if (commandName === 'rtk') {
      const wrapperCommand = extractFromRtkCommand(commandLine, node);
      if (wrapperCommand === null) {
        return [__.extractTextRange(commandLine, node.name.pos, node.end)];
      }
      return extractSimpleCommands(wrapperCommand);
    }
  ...
  /**
   * Extract the command after a transparent prefix (time)
   * @param {string} commandLine - The full command line
